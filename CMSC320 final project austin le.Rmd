...
## Map Plots 
Another way we can analyze our data is by using a map plot of the U.S. since our data provides us with pollution from each state. We'll be using R's function geom_map to create a map of the U.S.. Since we have data from 2000-2016, we'll look at 3 years: 2000, 2008, and 2016. Any state that has pollution data provided will be colored on the map. Any state that does not have pollution data will not be colored. We will have a legend that shows for the lighter color a state is, the more pollution they produced for that year. The two boxes in the corner are for Alaska and Hawaii.
...
##Hypotheis Testing and Machine Learning
